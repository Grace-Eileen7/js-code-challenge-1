...
function estimateTransactionFee() {
    ...
    const input = prompt("Enter amount wish to send KES):"); // Ask's the user to input how much money they want to send

    const amountToSend = Number(input); // Convert the input from text to a number

    if (isNaN(amountToSend) || amountToSend <= 0) {  // Check's if the input is a valid positive number
    ...
    let fee = amountToSend * feepercent; // Calculate's the fee based on the rate, using let cause the figure changes
    ...
    // If the fee is more than maximum, set's fee to maximum
    if (fee > maxFee) {
        fee = maxFee;
    }

    // Calculate's total amount to be debited (amount + transactionfee)
...
// Call's the function to run the estimator
